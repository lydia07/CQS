...
def make_para_dataset():
    ...
    test_src_file = "/data/hyx/workspace/CQS/dataset/SQuAD_t/para-test.txt"
    test_tgt_file = "/data/hyx/workspace/CQS/dataset/SQuAD_t/tgt-test.txt"
    ...
    word2idx = make_vocab(counter, src_word2idx_file, config.vocab_size)
    make_embedding(embedding_file, embedding, word2idx)

    # preprocess dev data
    dev_data = SquadData(ner_model=ner_model, data_dir=dev_squad)
    dev_test_pks, counter = dev_data.process_data()
    num_dev = len(dev_test_pks) // 2
    dev_pks = dev_test_pks[:num_dev]
    test_pks = dev_test_pks[num_dev:]
    dev_data.make_tag_format(dev_pks, dev_src_file, dev_tgt_file)
    dev_data.make_tag_format(test_pks, test_src_file, test_tgt_file)
